...
public class test2 {
	...
	public static void responseToClient(String request, PrintWriter out,OutputStream data)throws IOException{
		...
		try{
			...
			//Checking for Errors
			//IF there is an error -> send proper message to client
			//else procede to send the documents
			...
			*/
				...
				//in case of file being just a file and not a dirctory
				if (filepath.isFile() ) {
					//Use mime types to see what file is asked
					try{ //get suffix
					...
					//first check index.htm, if there is not , build an appropriate html file
					File indexHTML;
					...
					if (indexHTML != null) {//if index exists , call sendFile for it to send it
		...
		catch (Exception e) {                                      //Na GINEI IO EXCEPTION META?
			/*
